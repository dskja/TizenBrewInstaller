...
class SamsungCertificateCreator {
    ...
    async _fetchDistributorProfile(accessInfo, authorInfo, distributorCert) {
        const formData = new FormData();
        formData.append('access_token', accessInfo.accessToken);
        formData.append('user_id', accessInfo.userId);
        formData.append('platform', 'VD');
        ...
        // Use v1 endpoint for device-profile.xml (matches Samsung reference implementation)
        const request = await fetch('https://svdca.samsungqbe.com/apis/v1/distributors', {
            method: 'POST',
            headers: formData.getHeaders(),
            body: formData
        });
        ...
        if (request.ok) {
            ...
            throw new Error('Failed to fetch distributor profile\n' + await request.text());
        }
    }

    async _fetchDistributorCert(accessInfo, authorInfo, distributorCert) {
        // Reuse the same CSR and private key — matches Samsung reference implementation
        const formData = new FormData();
        formData.append('access_token', accessInfo.accessToken);
        formData.append('user_id', accessInfo.userId);
        formData.append('platform', 'VD');
        ...
        // Use v3 endpoint for signed certificate
    ...
    async createCertificate(authorInfo, accessInfo, duidList) {
        ...
        distributorCert.duidList = duidList;
        const authorCertVD = await this._fetchAuthorCert(accessInfo, authorCert);
        // First call to /distributors returns device-profile.xml
        const distributorXMLVD = await this._fetchDistributorProfile(accessInfo, authorInfo, distributorCert);
        // Second call to /distributors with a new CSR returns the signed distributor certificate
